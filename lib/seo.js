...
module.exports.init = function(setup) {
  ...
  app.get('/mqe/sitemap', function(req, res){
...
function generateStaticSnapshot(req, res) {
    ...
    if( !url.match(/\/?/) ) url += '/';
    url = url+'/'+((config.snapshot && config.snapshot.path) ? config.snapshot.path : '')+'#'+req.query._escaped_fragment_;
    ...
    cp.exec(__dirname+'/../node_modules/phantomjs/bin/phantomjs --ignore-ssl-errors=true '+__dirname+'/snapshot.js '+url,
        { encoding: 'utf8',
          timeout: 7000*60,
          killSignal: 'SIGKILL'
        },
        function (error, stdout, stderr) {
            if( error !== null ) {
                ...
                logger.error(error);
                return res.send({error: true, message: 'error generating snapshot', details: stderr});
