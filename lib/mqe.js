...
function handleQuery(req, res) {
  ...
  getResults(req, function(err, results){
      ...
      if( setup.process.query ) {
        setup.process.query(req.query, results.items, function(items){
          results.items = items;
          res.send(results);
...
function handleGet(req, res) {
  ...
  getItem(req, function(err, result){
      ...
      // user can override this
      if( setup.process.get ) {
        setup.process.get(req.query, result, function(result){
          res.send(result);
...
function rangedQuery(options, query, callback) {
  ...
  if (query.projection) {
     // qjhart is disabling the whitelist feature.  jrmerz sez we should remove it entirely, but
     // it's also possible to separate the whiteList and the Default projection.
     // To renable, rm the && false
     if (config.db.searchWhitelist && false) {
         white_filters=filters;
	 filters={};
	 for (var key in query.projection) {
	     if (query.projection.hasOwnProperty(key)) {
		 key_a=key.split('.');
		 logger.info('key'+key_a);
		 do {
                     logger.info('test '+key+' '+key_a.join('.'));
		     if (white_filters[key_a.join('.')]) {
			 filters[key]=query.projection[key];
		     }
		 } while (key_a.pop());
	     }
	 }
